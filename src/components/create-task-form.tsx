...
export function CreateTaskForm({ columns }: CreateTaskFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="rounded-3xl border border-white/5 bg-black/30 p-6 text-sm text-white">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <p className="text-xs uppercase tracking-[0.4em] text-zinc-500">Create Task</p>
          <h2 className="text-xl font-semibold text-white">Add a new Kanban item</h2>
        </div>
        <button
          type="submit"
          className="rounded-full bg-sky-500 px-5 py-2 text-sm font-semibold text-white shadow-lg shadow-sky-500/30 disabled:opacity-60"
      ...
      <div className="mt-4 grid gap-4 md:grid-cols-2">
        <div className="space-y-2">
          ...
          <input
            className="w-full rounded-2xl border border-white/5 bg-black/40 px-4 py-2 text-sm text-white"
            value={title}
            onChange={(event) => setTitle(event.target.value)}
            placeholder="e.g., Outreach follow-up sequence"
            required
          />
        </div>
        <div className="space-y-2">
      ...
      <div className="mt-4 space-y-2">
        ...
        <textarea
          className="min-h-[100px] w-full rounded-2xl border border-white/5 bg-black/40 px-4 py-3 text-sm text-white"
          value={description}
          onChange={(event) => setDescription(event.target.value)}
          placeholder="What needs to happen? Include context or links."
          required
        />
      </div>
